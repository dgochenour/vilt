...
BrakeCommand::BrakeCommand (
    ...
    :
        m_device_id_( device_id ),
        m_brake_intensity_( brake_intensity ) {
}

#ifdef RTI_CXX11_RVALUE_REFERENCES
#ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
BrakeCommand::BrakeCommand(BrakeCommand&& other_) OMG_NOEXCEPT  :m_device_id_ (std::move(other_.m_device_id_))
,
m_brake_intensity_ (std::move(other_.m_brake_intensity_))
{
} 

BrakeCommand& BrakeCommand::operator=(BrakeCommand&&  other_) OMG_NOEXCEPT {
    BrakeCommand tmp(std::move(other_));
    swap(tmp); 
    return *this;
}
#endif
...
void BrakeCommand::swap(BrakeCommand& other_)  OMG_NOEXCEPT 
{
    using std::swap;
    swap(m_device_id_, other_.m_device_id_);
    swap(m_brake_intensity_, other_.m_brake_intensity_);
}  

bool BrakeCommand::operator == (const BrakeCommand& other_) const {
    if (m_device_id_ != other_.m_device_id_) {
        return false;
    }
    if (m_brake_intensity_ != other_.m_brake_intensity_) {
        return false;
    }
    return true;
}
...
std::ostream& operator << (std::ostream& o,const BrakeCommand& sample)
{
    ::rti::util::StreamFlagSaver flag_saver (o);
    o <<"[";
    o << "device_id: " << sample.device_id()<<", ";
    o << "brake_intensity: " << std::setprecision(9) <<sample.brake_intensity() ;
    o <<"]";
    return o;
}
...
namespace rti { 
    namespace topic {
        ...
        template<>
        struct native_type_code< BrakeCommand > {
            static DDS_TypeCode * get()
            {
                using namespace ::rti::topic::interpreter;
                ...
                static DDS_TypeCode BrakeCommand_g_tc_device_id_string;
                ...
                BrakeCommand_g_tc_device_id_string = initialize_string_typecode(((DEVICE_ID_MAX_LENGTH)));
                ...
                BrakeCommand_g_tc._data._sampleAccessInfo = sample_access_info();
                BrakeCommand_g_tc._data._typePlugin = type_plugin_info();    
            ...
            static RTIXCdrSampleAccessInfo * sample_access_info()
            {
                ...
                BrakeCommand_g_sampleAccessInfo.useGetMemberValueOnlyWithRef =
                RTI_XCDR_TRUE;

                BrakeCommand_g_sampleAccessInfo.getMemberValuePointerFcn = 
                interpreter::get_aggregation_value_pointer< BrakeCommand >;
        ...
        const ::dds::core::xtypes::StructType& dynamic_type< BrakeCommand >::get()
        {
            return static_cast<const ::dds::core::xtypes::StructType&>(
                ::rti::core::native_conversions::cast_from_native< ::dds::core::xtypes::DynamicType >(
                    *(native_type_code< BrakeCommand >::get())));
        }
...
namespace dds { 
    namespace topic {
        ...
        void topic_type_support< BrakeCommand >:: register_type(
            ::dds::domain::DomainParticipant& participant,
            const std::string& type_name) 
        {
            ...
            ::rti::domain::register_type_plugin(
                participant,
                type_name,
                BrakeCommandPlugin_new,
                BrakeCommandPlugin_delete);
        }

        std::vector<char>& topic_type_support< BrakeCommand >::to_cdr_buffer(
            std::vector<char>& buffer, 
            ...
            ::dds::core::policy::DataRepresentationId representation)
        {
            // First get the length of the buffer
            unsigned int length = 0;
            RTIBool ok = BrakeCommandPlugin_serialize_to_cdr_buffer(
                NULL, 
                &length,
                &sample,
                representation);
            ...
            // Create a vector with that size and copy the cdr buffer into it
            buffer.resize(length);
            ok = BrakeCommandPlugin_serialize_to_cdr_buffer(
                &buffer[0], 
                &length, 
                &sample,
                representation);
        ...
        void topic_type_support< BrakeCommand >::from_cdr_buffer(BrakeCommand& sample, 
        const std::vector<char>& buffer)
        {

            RTIBool ok  = BrakeCommandPlugin_deserialize_from_cdr_buffer(
                &sample, 
                &buffer[0], 
                static_cast<unsigned int>(buffer.size()));
            ::rti::core::check_return_code(ok ? DDS_RETCODE_OK : DDS_RETCODE_ERROR,
            "Failed to create BrakeCommand from cdr buffer");
        }

        void topic_type_support< BrakeCommand >::reset_sample(BrakeCommand& sample) 
        {
            sample.device_id("");
            sample.brake_intensity(0.0f);
        }

        void topic_type_support< BrakeCommand >::allocate_sample(BrakeCommand& sample, int, int) 
        {
            RTIOsapiUtility_unusedParameter(sample); // [[maybe_unused]]
